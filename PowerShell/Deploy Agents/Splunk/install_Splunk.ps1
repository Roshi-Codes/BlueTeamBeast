...
# You must replace the below url with the url and name of your hosted splunkforwarder - simple python http server will work.
$url = "http://<URL>/splunkforwarder.msi" 
...
filetest
if ($test = "False"){Write-Warning "Download unsucessful, Check connectivity and file availibilty, then try installing on $env:COMPUTERNAME again. Leaving session with $env:COMPUTERNAME.."; Exit}
}


function filetest {
Global $test = Test-Path C:\splunkforwarder.msi
if ($test = "True"){Continue}
elif ($test = "False"){Write-Warning "C:\splunkforwarder.msi NOT Found"}
}
